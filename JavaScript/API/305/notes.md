# API 305
## JavaScript's `.fetch` API is one way to execute common HTTP requests. 

**`GET`** 
```javascript
async function getData(){
const data = await fetch('https://sj-demo-contacts.herokuapp.com/contacts?auth=5x76')
.then(response => response.json()) // this comes back with some data we cannot read so we convert to json
.catch(err => comsole.error(err))
console.log(data);
}
```
**`DELETE`**
```javascript
async function deleteData(id){ // id will need to be passed in 
const data = await fetch(`https://sj-demo-contacts.herokuapp.com/contacts/delete/${id}?auth=5x76`, {
    headers: {
        'content-type': 'application/json'
    },
    method: 'DELETE', 
}) // will need to add delete and id 
.then(response => response.json()) // this comes back with some data we cannot read so we convert to json
.catch(err => comsole.error(err))
console.log(data);
}


```
**`POST`**   
```javascript
async function postData(body){ // contains the information that we are sending
const data = await fetch(`https://sj-demo-contacts.herokuapp.com/contacts/delete/${id}?auth=5x76`, {
    body: JSON.stringify(body),
    headers: {
        'content-type': 'application/json'
    },
    method: 'POST', 
}) 
.then(response => response.json()) 
.catch(err => comsole.error(err))
console.log(data);
}
```
**`PUT`** 
```javascript
async function updateData(bod, id){ 
const data = await fetch(`https://sj-demo-contacts.herokuapp.com/contacts/${id}?auth=5x76`, {
    body: JSON.stringify(body),
    headers: {
        'content-type': 'application/json'
    },
    method: 'PUT', 
}) 
.then(response => response.json()) 
.catch(err => comsole.error(err))
console.log(data);
}


```